  ...
  public void run()
  {
    ...
    while(f.b  && !estSuppr)
    {
      ...
      while(f.b && threadState==1 && !estSuppr)
      {
          ...
          {
            // Le thread regarde toutes les secondes si le niveau est réussi :
        	  //cad si le niveau contient des Instances de Ballons et qu'elles sont toutes soit en état "n'existe plus" soit inactives :
            t=System.currentTimeMillis();
            if(f.annuaire.containsKey("Ballons"))
            {
            	...
            	{
            		f.win.setVisible(true);// Lance la victoire en rendant visible (et cliquable) le boutons géant win
            	}
            }
